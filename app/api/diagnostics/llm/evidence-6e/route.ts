...
const RESULT_PATH = path.join(process.cwd(), 'diagnostic-fixtures', 'test6e-ahroon-evidence-gaps.json')
const EXPECTED_IDS = [
...
// TEST 6E experimental variables (vs TEST 6C and 6D):
//  - TEST 6C used a 1,600-token output ceiling and exhausted it in reasoning (finish_reason: length).
//  - TEST 6D used max_tokens 8,192, which pushed reserved request (~input + 8,192) past the
//    8,000 TPM account gate -> HTTP 413 before generation. The model never ran.
//  - TEST 6E uses max_tokens 3,000: substantially MORE room than 6C's 1,600, while keeping the
//    reserved request comfortably below the 8,000 TPM limit. This is the proper test of the
//    6C hypothesis (that empty message.content was caused by too small an output budget).
// Text generation, no native JSON-schema enforcement, single request, all 7 posts supplied,
// no semantic/repost/six-month filtering are all held constant.
...
export async function POST() {
  ...
  try {
    ...
    return NextResponse.json({ test: 'TEST 6E', status: 'FAIL', error: error instanceof Error ? error.message : 'Fixture unavailable', brightDataCalls: 0, llmCalls: 0, databaseMutations: 0 }, { status: 422 })
  }
  ...
  // --- Deterministic preflight budget check (char/4 heuristic; no tokenizer installed) ---
  const estimatedInputTokens = Math.ceil(`${SYSTEM_PROMPT}\n${userPrompt}`.length / 4)
  const reservedOutputTokens = MAX_OUTPUT_TOKENS
  const estimatedTotalReserved = estimatedInputTokens + reservedOutputTokens
  const preflightGreen = estimatedTotalReserved < GROQ_TPM_LIMIT
  const requestMetadata = {
    provider: 'Groq', model: MODEL, responseFormat: 'text',
    maxOutputTokens: MAX_OUTPUT_TOKENS, reasoningEffort: REASONING_EFFORT, postsSupplied: 7,
    preflight: {
      estimatedInputTokens, reservedOutputTokens, estimatedTotalReserved,
      groqTpmLimit: GROQ_TPM_LIMIT, classification: preflightGreen ? 'GREEN' : 'RED',
    },
  }

  if (!preflightGreen) {
    const result = {
      test: 'TEST 6E', capturedAt: new Date().toISOString(), sourceFixture: 'diagnostic-fixtures/test4d-ahroon-compacted.json', postIdsSupplied: input.ids,
      request: requestMetadata,
      response: null,
      validation: { preflight: 'RED', note: 'Preflight estimated reserved request >= 8,000 TPM. Groq request NOT made. No automatic budget change, no retry.' },
      boundaries: { brightDataCalls: 0, llmCalls: 0, databaseMutations: 0, productionPipeline: false, askGeneration: false, finalSynthesis: false, semanticFilteringBeforeLlm: false, repostFiltering: false, retryOrRepair: false },
      status: 'BLOCKED',
      failureReason: `Preflight RED: estimated reserved request ${estimatedTotalReserved} >= TPM limit ${GROQ_TPM_LIMIT}.`,
    }
  ...
  if (!process.env.GROQ_API_KEY) {
    return NextResponse.json({ test: 'TEST 6E', status: 'FAIL', error: 'GROQ_API_KEY is not configured.', request: requestMetadata, brightDataCalls: 0, llmCalls: 0, databaseMutations: 0 }, { status: 502 })
  }
  ...
  try {
    ...
    const result = {
      test: 'TEST 6E', capturedAt: new Date().toISOString(), sourceFixture: 'diagnostic-fixtures/test4d-ahroon-compacted.json', postIdsSupplied: input.ids,
      request: requestMetadata, response: null,
      validation: { transport: 'FAIL' },
      ...
      status: 'FAIL', failureReason: error instanceof Error ? error.message : 'LLM request failed',
    }
    await writeFile(RESULT_PATH, JSON.stringify(result, null, 2))
  ...
  const notLengthTruncated = finishReason !== 'length'
  const structuredValid = Boolean(response.ok && content && String(content).trim() && notLengthTruncated && validShape && validCitations)
  ...
  const result = {
    ...
    validation: {
      ...
      finishReasonNotLength: notLengthTruncated,
    ...
    failureReason: structuredValid ? null : response.ok ? (content ? (notLengthTruncated ? 'message.content present but JSON invalid or cited an unsupplied post ID.' : 'finish_reason: length — output budget still exhausted.') : `message.content empty (finish_reason: ${finishReason}).`) : 'Groq provider request failed.',
  }
